...
ggplot() +
  ...
  geom_text(data = level4_trans,mapping = aes(x = Year, y = value / 10^6,label = ifelse(percent > 2, paste0(percent, "%"), "")),
...
power <- read_excel("data/Niti_dashboard_elec_gen_utility.xlsx", na = "-") %>%
...
power_captive <- read_excel("data/Niti_captive_gen_emissions.xlsx", na = "-") %>%
  mutate(Unit = "GWh") %>%
...
ggplot() +
  geom_col(power_captive, mapping = aes(x = Year, y = value / 1000, fill = Source)) +
  geom_text(power_captive,
    mapping = aes(x = Year, y = value / 1000, fill = Source, label = ifelse(percent > 3, paste0(round(percent, 0), "%"), "")),
    position = position_stack(vjust = 0.5), size = 3, color = "black"
  ) + # Adjust the position and size of the text
  geom_text(
    data = power_captive_total, aes(x = Year, y = total_value / 1000, label = round(total_value / 1000, 1)),
    vjust = -0.5, size = 3, color = "black", fontface = "bold"
  ) + # Add total values on top of bars
...
power_consum <- read_excel(path = "data/Niti_elec_consumption.xlsx") %>%
...
ggplot() +
  geom_col(power_consum, mapping = aes(x = Year, y = value / 1000, fill = Category)) +
  geom_text(power_consum,
    mapping = aes(x = Year, y = value / 1000, fill = Category, label = ifelse(percent > 3, paste0(round(percent, 0), "%"), "")),
    position = position_stack(vjust = 0.5), size = 3, color = "black"
  ) + # Adjust the position and size of the text
  geom_text(
  ...
  labs(x = "", y = "Electricity Consumption (BU)") +
...
niti <- read_excel("data/Niti_GVA.xlsx") %>%
  select(-percent) %>%
...
ggplot() +
  geom_col(niti, mapping = aes(x = Year, y = value / 100000, fill = Sector_Category)) +
  geom_text(niti,
    mapping = aes(x = Year, y = value / 100000, fill = Sector_Category, label = ifelse(percent > 2, paste0(round(percent, 0), "%"), "")),
    position = position_stack(vjust = 0.5), size = 3, color = "black"
  ) +
  geom_text(data = niti_tot, mapping = aes(x = Year, y = value / 100000, label = round(value / 100000, 0), vjust = -0.5, fontface = "bold")) +
...
veh_data_comb <- left_join(veh_data_filter,veh_data_filter2,by=c("Category")) %>% 
  mutate(increase=value.x/value.y) %>% filter(Year.x>2009)
